...
pub trait AgentAdapter: Send + Sync {
    ...
    /// Write the tool's hook config into the fresh worktree so its hooks call `clowder-hook`.
...
/// absolute path. Order: `$CLOWDER_HOOK_BIN`, then a sibling of the running daemon executable
/// (`target/debug/clowder-hook` next to `clowder-daemon`), then a bare `clowder-hook` (assume PATH).
pub(crate) fn clowder_hook_bin() -> String {
    if let Ok(p) = std::env::var("CLOWDER_HOOK_BIN") {
        if !p.is_empty() {
            return p;
        }
    }
    if let Ok(exe) = std::env::current_exe() {
        if let Some(sibling) = exe.parent().map(|d| d.join("clowder-hook")) {
            if sibling.exists() {
                return sibling.to_string_lossy().into_owned();
            }
        }
    }
    "clowder-hook".to_string()
}

/// Real Claude Code adapter: writes .claude/settings.local.json whose Notification/Stop
/// hooks invoke `clowder-hook`, plus a .claude/.gitignore so that hook config isn't committed
...
/// via `-c`. clowder-hook self-IDs from the CLOWDER_AGENT_ID/CLOWDER_HOOK_SOCK env the daemon injects
...
impl AgentAdapter for CodexAdapter {
    ...
    fn launch_command(&self, _worktree: &Path) -> PaneCommand {
        ...
        // clowder-hook path so a path containing spaces still parses.
...
impl AgentAdapter for SyntheticAdapter {
    ...
    fn provision_hooks(&self, worktree: &Path, agent_id: PaneId, _hook_sock: &Path) -> Result<()> {
        std::fs::write(worktree.join(".clowder-agent"), agent_id.0.to_string())?;
        Ok(())
    }
...
mod tests {
    ...
    fn claude_adapter_writes_hook_settings() {
        ...
        // Notification + Stop hooks both invoke clowder-hook with the right event. The binary
        ...
        assert!(notif.contains("clowder-hook") && notif.ends_with("--event notification"), "got: {notif}");
        assert!(stop.contains("clowder-hook") && stop.ends_with("--event stop"), "got: {stop}");
    ...
    #[test]
    fn codex_launch_command_wires_notify_to_clowder_hook() {
        let cmd = CodexAdapter.launch_command(std::path::Path::new("/tmp/ws"));
        assert_eq!(cmd.program, "codex");
        let bin = crate::agent::clowder_hook_bin();
