...
- **Client strategy:** NATIVE per platform — SwiftUI/AppKit on macOS (built first), `gtk4-rs` on Linux (later). Both are thin views speaking `muxy-proto` to the shared Rust daemon. The core/daemon stays cross-platform Rust regardless.
...
- **M5 — Robustness.** Scrollback caps + reflow-on-resize, crash/reap handling, daemon persistence/restart, and config files. (The `muxy-proto` `Transport` trait stays the pluggable network seam the remote milestone builds on.)
- **M6 — Build & packaging (macOS release).** Turn the dev-only build into a distributable, CI-produced app: wrap the release `MuxyApp` in a real `.app` bundle (`Info.plist`, bundle id, icon, version — retiring the bare-exe `setActivationPolicy` workaround); bundle `muxy-daemon`/`muxy`/`muxy-hook` and resolve them **bundle-relative** (dropping `MUXY_BIN` and the `../target/debug` fallback), with the app **launching/supervising its own daemon** over **per-user sockets** (not shared `/tmp`); a committed script that reproducibly builds/vendors the pinned **libghostty** (`zig 0.16` + the `build.zig` Darwin patch, ghostty `2de5e7d`); Developer ID **codesign → notarize → staple → DMG**; real versioning + git-tagged releases; a **Homebrew cask** (in a tap, referencing the notarized release artifact and auto-bumped by the release CI, so `brew install --cask muxy` installs the app); and **GitHub Actions CI** (libghostty + `cargo test` + `swift test` + the signed artifact), closing the standing "libghostty build in CI" risk gate. (Sparkle auto-update + Linux packaging deferred.) Note: since the macOS app already works end-to-end (M0–M4), packaging can be pulled ahead of the other remaining milestones for a shippable build sooner.
- **M7 — Remote daemon (connect from another machine).** Let the daemon run on a remote host and the macOS app attach over the network. Swap the local `UnixSocketTransport` for an authenticated, encrypted network transport via the `muxy-proto` `Transport` trait (`QuicTransport`/TLS); the daemon binds a **network listener** (not just `/tmp` Unix sockets) and the client connects by **host address + credentials** (config + a pairing/token auth flow), so the control, render (`muxy attach <pane>`), and byte streams tunnel over the secured link with reconnect/latency handling. Agents, worktrees, and hooks stay local to the daemon host. Realizes the north-star's remote seam for a remote desktop daemon. (Mobile/phone client stays post-v1.)
- **M8 — Linux client.** `gtk4-rs` client over the same `muxy-proto`; libnotify + tray; libghostty via GLArea + `muxy-ghostty-sys`.

**Deliberately post-v1 (YAGNI):** phone/mobile push (until the M7 remote seam extends to it), dashboard-grid overview mode, plugin API. (Note: splits are *in* v1 but scoped to companion panes within one workspace — free-form tiling of unrelated agents together stays out.)
...
- **M6 packaging:** a clean checkout (no vendored `.a`) builds a **signed + notarized `muxy.dmg`** via the committed libghostty script + CI; installing the `.app` and double-clicking it launches the daemon and spawns agents with **no** env vars, no manual daemon, and no `cargo`/`swift run`; CI is green on macOS; and `brew install --cask <tap>/muxy` installs a launchable, notarized app.
- **M7 remote:** with the daemon running on a second machine, the macOS app connects to it by address with authentication over the encrypted transport, spawns/attaches agents, and drives terminals live — the agents' worktrees live on the remote host, and the link survives a brief network drop (reconnect).
- **Risk gates:** M0 does not close until native libghostty embedding + detached survival are demonstrated on macOS; the Zig/libghostty build runs in CI on both OSes before the Linux client (M8).
