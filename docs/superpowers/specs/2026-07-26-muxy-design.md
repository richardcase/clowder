...
- **M7 packaging:** a clean checkout (no vendored `.a`) builds a **signed + notarized `muxy.dmg`** via the committed libghostty script + CI; installing the `.app` and double-clicking it launches the daemon and spawns agents with **no** env vars, no manual daemon, and no `cargo`/`swift run`; CI is green on macOS.
